...
module Make (Z : Bignum.S_EXTENDED) (R : RandomSig.S) = struct
  ...
  let rec random_bigint_at_depth (state : R.t) depth : Z.t =
    if depth = 0 then
      Z.of_int (R.bits state)
    else
  ...
  let random state range =
    if Z.leq range Z.zero then
      ...
      Z.of_int (R.int state (Z.to_int range))
    else
