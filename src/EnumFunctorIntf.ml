...
(** The signature for enumerations.
...
*)
module type S = sig
  module Ifseq : IFBigSeqSig.IFBIGSEQ
...
(** The interface for an enumeration functor module. *)
module type Intf = sig
  module type S = S

  (** Makes an enumeration module.

    @param Ifseq an implicit finite sequences module
  *)
  module Make (Ifseq : IFBigSeqSig.IFBIGSEQ) : S with module Ifseq = Ifseq
end
